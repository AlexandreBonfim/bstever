...
const getSuggestions =  (user: User, clubId: number): Suggestion[] => {
  ...
  */
  const booksFromAuthorsPriority = sortByYear(books.filter(({ authorId }) => (priority(clubId).authorIds).includes(authorId)))
  ...
  *   If none are shared, fallback to most popular by all Users.
  */
  const booksFromGenresPriority = sortByYear(books.filter(({ genres }) => genres.some(genre => priority(clubId).genres.includes(genre))))
  ...
  */
  const booksFromAuthorsNotShared = sortByYear(books.filter(({ authorId }) => !(priority(clubId).authorIds).includes(authorId)))
...
export const exportedForTesting = { sortByYear, priority, formattedResponse }
